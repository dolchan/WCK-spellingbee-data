...
read_word_list <- function(filename) {
  ...
  # actually, this is not working properly....
  ...
  # split multiple words in a line
  # multi_words_loc <- grep(" ", x)
  # if (length(multi_words_loc) > 0) {
  #  multi_words_lines <- x[multi_words_loc]
  #  x <- x[-multi_words_loc]
  #  
  #  multi_words_lines %>% 
  #    sub(" +$", "", .) %>%
...
```{r}
words_of_school <- read_word_list("2022-word-list-school.txt")
```

```{r}
words_of_school %>%
  select(word) %>%
  left_join(words_of_champions, by = "word") ->
  words_of_school
```
